...
#
# Stage 1  Train LeWM v2 (mixed random + PPO, predictor hidden dim 384)
...
#
#   SKIP_COLLECT=1   skip Stage 0 (e.g. random_500k.pkl already exists)
...
#   COLLECT_STEPS          random steps to collect  (default: 500000)
...
#   RANDOM_BUFFER          (default: data/crafter/ppo_rollouts/crafter_data.pkl)
...
RANDOM_BUFFER="${RANDOM_BUFFER:-${COLLECT_OUTPUT}}"
...
COST="${COST:-probe}"
WANDB="${WANDB:-0}"
[[ "${WANDB}" == "1" ]] && WANDB_FLAG="--wandb" || WANDB_FLAG=""

# Resolved absolute paths
LOGDIR_ABS="$(resolve_repo_path "${LOGDIR}")"
LEWM_CKPT="${LEWM_CKPT:-${LOGDIR_ABS}/best.pt}"
# RANDOM_ABS is set after Stage 0 (so it picks up the collected file)
...
if [[ "${SKIP_COLLECT}" == "1" ]]; then
    echo "=== Stage 0: SKIPPED (SKIP_COLLECT=1) ==="
    if [[ ! -f "${COLLECT_OUTPUT_ABS}" ]]; then
        echo "ERROR: COLLECT_OUTPUT not found: ${COLLECT_OUTPUT_ABS}"
        exit 1
    fi
    ...
    echo "  output: ${COLLECT_OUTPUT_ABS}"
...
# RANDOM_BUFFER now points at the collected file (unless user overrode it)
...
if [[ "${SKIP_EVAL}" == "1" ]]; then
    ...
    for cond in hwm hwm_oracle; do
        echo "--- condition: ${cond} ---"
        extra_flags=(); [[ "${cond}" != "hwm" ]] && extra_flags=(--append)
        "${PY}" hwm/evaluate.py \
            --condition      "${cond}" \
            --checkpoint     "${LEWM_CKPT}" \
