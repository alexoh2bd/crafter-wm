...
from hwm.constants import ACTION_DIM, CHECKPOINT, CONTEXT_LEN, LATENT_DIM
...
class ActionEncoder(nn.Module):
    ...
    (no causal mask) + linear projection to LATENT_DIM.
    ...
    Args:
        ...
        latent_dim: Output macro-action dimension (default 256, matching LeWM).
    ...
    def __init__(
        ...
        # Project CLS output to macro-action space
    ...
    def forward(self, a_seq: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            l: (B, latent_dim) macro-action vector.
        """
        B, L, _ = a_seq.shape
...
class HighLevelPredictor(nn.Module):
    ...
    def __init__(
        ...
        # Macro-action → conditioning signal (same interface as low-level,
        # but input is already latent_dim instead of action_dim)
